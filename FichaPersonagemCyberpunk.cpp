#include <iostream> // Trabalho realizado por Leandra e João Paulo
#include <string>

int main() {
    // Dados do personagem
    std::string predefinidos[40] = {
        "Nome do Personagem", // texto pré definido para ser digitado outro texto posteriormente
        "Apelido",
        "Lugar de Origem",
        "Data de Nascimento",
        "Sexo do Personagem",
    ...
    std::cout << "*CRIE A FICHA DO PERSONAGEM*:\n";

    for (int i = 0; i < 40; ++i) {
        std::cout << "Linha " << i + 1 << ": " << predefinidos[i] << " - ";
        std::getline(std::cin, entradaUsuario); // para permitir que o usuário digite frases com espaços
